...
class TestGradBoosting(unittest.TestCase):
    ...
    def testSmoothQuantileReg(self):
        """
        @brief test ability to estimate conf intervals by the quantile loss function
        """
        np.random.seed(1)
        ...
        # fit to median
        gbt = GBRTmodel(max_depth=1, learning_rate=0.2, subsample=0.9, loss="smth_quantile", alpha=0.5)
        gbt.train(X, y, n_estimators=350)
        y_median = gbt.predict(xx)
        ...
        if MPL:
            ...
            plt.ylim(-10, 20)
            plt.legend(loc='upper left')
            plt.savefig('1d_boosted_regression_smth_quantile_ex.png')
            plt.close()
